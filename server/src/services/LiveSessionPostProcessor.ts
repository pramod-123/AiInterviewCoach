...
export class LiveSessionPostProcessor {
  ...
  async run(sessionId: string, options?: LiveSessionPostProcessRunOptions): Promise<void> {
          ...
          this.log.info(
            {
              sessionId,
              jobId,
              geminiChunks: stitched.chunkCount,
            },
    ...
    const allowGeminiRealtimeTranscript = liveInterviewerEnabled && !forceLocalStt;
    ...
    try {
      if (geminiBundle && geminiBundle.utterances.length > 0) {
        ...
        this.log.debug(
          ...
          "Live session transcript from live realtime (input/output transcription).",
        );
      }
