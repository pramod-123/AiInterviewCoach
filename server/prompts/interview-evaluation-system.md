...
- Put the **complete editor source** from that moment in the `quote` field—the same full buffer you would use from the evaluation input / transcript timeline for that time, **not** a single line, a fragment, or a “representative” excerpt.
- Set `timestamp_ms` to the **recording-start milliseconds** for that snapshot (aligned with code snapshot / timeline times).
