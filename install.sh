#!/usr/bin/env bash
# Install Node dependencies, apply Prisma schema, and start the API server.
# Run from anywhere: ./install.sh   or   bash install.sh
...
if [[ ${#missing_bin[@]} -gt 0 ]]; then
  ...
  echo "  macOS: brew install ffmpeg tesseract   (or re-run: ./install.sh --brew)" >&2
